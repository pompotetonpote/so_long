/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   imgxpm.h                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: pompote <pompote@student.42.fr>            +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/02/14 14:01:28 by yperonne          #+#    #+#             */
/*   Updated: 2023/02/20 10:57:41 by pompote          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#ifndef IMGXPM_H
# define IMGXPM_H

/* CHARACTER */
# define BACK1      "./img/satoshi_back1.xpm"
# define BACK2      "./img/satoshi_back2.xpm"
# define FRONT1     "./img/satoshi_front1.xpm"
# define FRONT2     "./img/satoshi_front2.xpm"
# define STATICL    "./img/satoshi_static_left.xpm"
# define STATICR    "./img/satoshi_static_right.xpm"
# define LEFT1      "./img/satoshi_walk_left1.xpm"
# define LEFT2      "./img/satoshi_walk_left2.xpm"
# define RIGHT1     "./img/satoshi_walk_right1.xpm"
# define RIGHT2     "./img/satoshi_walk_right2.xpm"

/* FEATURES*/
# define WALL       "./img/wall.xpm"

# define VORTEX     "./img/vortex.xpm"

# define BLOCK      "./img/satoshiblock_solong.xpm"

# define FLOOR      "./img/pixel_soil_green.xpm"

#endif